...
const EMPTY = { nom:'', zone:'Souss-Massa', ville:'', tel:'', adresse:'', ice:'' }
...
export default function Clients({ showToast, search }) {
  ...
  function openEdit(c) { setForm({ nom:c.nom, zone:c.zone, ville:c.ville||'', tel:c.tel||'', adresse:c.adresse||'', ice:c.ice||'' }); setEditId(c.id); setModal(true) }
  ...
  return (
    <div>
      ...
      <div className="page-header-agro">
        ...
        <button className="btn-primary-agro" onClick={openAdd}><i className="bi bi-plus-lg"></i> Nouveau Client</button>
      </div>
      ...
      {modal && (
        <div className="modal-overlay-agro" onClick={e=>e.target===e.currentTarget&&setModal(false)}>
          <div className="modal-agro">
            <div className="modal-title-agro"><i className="bi bi-person-fill" style={{color:'var(--primary)'}}></i>{editId?'Modifier Client':'Nouveau Client'}</div>
            <div className="row g-3">
              <div className="col-12"><label className="form-label-agro">Nom *</label><input className="form-control-agro" value={form.nom} onChange={e=>setForm(f=>({...f,nom:e.target.value}))} placeholder="Ferme du Souss" /></div>
              <div className="col-6">
              ...
              <div className="col-6"><label className="form-label-agro">Ville</label><input className="form-control-agro" value={form.ville} onChange={e=>setForm(f=>({...f,ville:e.target.value}))} placeholder="Agadir" /></div>
              <div className="col-6"><label className="form-label-agro">Téléphone</label><input className="form-control-agro" value={form.tel} onChange={e=>setForm(f=>({...f,tel:e.target.value}))} placeholder="0661234567" /></div>
              <div className="col-6"><label className="form-label-agro">ICE</label><input className="form-control-agro" value={form.ice} onChange={e=>setForm(f=>({...f,ice:e.target.value}))} placeholder="001234567000001" maxLength={15} /></div>
              <div className="col-12"><label className="form-label-agro">Adresse</label><input className="form-control-agro" value={form.adresse} onChange={e=>setForm(f=>({...f,adresse:e.target.value}))} placeholder="Km 12 Route Taroudant" /></div>
            </div>
